...
2. **Self-audit scoring** — rate the epic on 13 quality dimensions, producing an overall 0–100 score
...
**auditChecks**: Run ALL 13 quality checks listed below. Each check is scored 0–10 and marked pass/fail (pass = score >= 7).

**overallScore**: Weighted average of all audit check scores, normalized to 0–100. Formula: (sum of all check scores / (13 × 10)) × 100, rounded to nearest integer.
...
<audit_checks>
Perform ALL 13 of the following quality checks. Score each 0–10.
...
3. Run all 13 audit checks, scoring each 0–10.
...
- All 13 audit checks must be present in the output.
